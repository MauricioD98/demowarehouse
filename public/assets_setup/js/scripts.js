$(function() {
    ...
    $('#db_connection').on("change", function(e) {
        var dbType = $(this).val();
        var portInput = $('#db_port');
        
        if (dbType === 'pgsql') {
            if (!portInput.val() || portInput.val() === '3306') {
                portInput.val('5432');
            }
            portInput.attr('placeholder', '5432');
        } else if (dbType === 'mysql') {
            if (!portInput.val() || portInput.val() === '5432') {
                portInput.val('3306');
            }
            portInput.attr('placeholder', '3306');
        }
    });
});
